        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
                    ...
                    <div v-if="drainase.data.length>0">
                        <table class="table-fixed w-full">
                            <thead>
                            <tr class="bg-gray-100">
                                <th class="px-4 py-2 w-20">No.</th>
                                <th class="px-4 py-2">Nama Kecamatan</th>
                                <th class="px-4 py-2">Kapasitas Drainase</th>
                                <th class="px-4 py-2">Aksi</th>
                            </tr>
                            </thead>
                            <tbody>
                            <tr v-for="row in drainase.data">
                                <td class="border px-4 py-2">{{ row.id }}</td>
                                <td class="border px-4 py-2">{{ row.kecamatan.kec_nama }}</td>
                                <td class="border px-4 py-2">{{ row.kapasitas_drainase }}</td>
                        ...
                        <pagination class="mt-6" :links="drainase.links"/>
                    </div>
                    <div v-else>
                                ...
                                <form>
                                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                        <div class="">
                                            ...
                                            <div class="mb-4">
                                                <label for="exampleFormControlInput1"
                                                       class="block text-gray-700 text-sm font-bold mb-2">Kapasitas
                                                    Drainase:</label>
                                                <input type="text"
                                                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                                       id="exampleFormControlInput1"
                                                       placeholder="Masukkan Kapasitas Drainase"
                                                       v-model="form.kapasitas_drainase">
...
export default {
    ...
    props: ['drainase', 'kecamatan', 'errors'],
    data() {
        return {
            editMode: false,
            isOpen: false,
    ...
    methods: {
        searchData: function () {
            this.$inertia.get(`/persebaran-drainase?cari=${this.query}`)
        },
        clearSearch: function () {
            this.query = '';
            this.$inertia.get(`/persebaran-drainase`)
        },
        openModal: function () {
            this.isOpen = true;
        },
        ...
        save: function (data) {
            this.$inertia.post('/persebaran-drainase', data)
            this.reset();
            this.closeModal();
            this.editMode = false;
        },
        ...
        update: function (data) {
            this.$inertia.post('/persebaran-drainase/' + data.id, {id_kecamatan : data.id_kecamatan, kapasitas_drainase : data.kapasitas_drainase, _method : 'PUT'})
            this.reset();
            this.closeModal();
        },
        deleteRow: function (data) {
            ...
            data._method = 'DELETE';
            this.$inertia.post('/persebaran-drainase/' + data.id, data)
            this.reset();
            this.closeModal();
        }
    }
