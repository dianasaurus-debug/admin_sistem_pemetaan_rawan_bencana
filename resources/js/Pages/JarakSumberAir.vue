        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
                    ...
                    <div v-if="jarak_sumberair.data.length>0">
                    <table class="table-fixed w-full">
                        ...
                        <tr class="bg-gray-100">
                            ...
                            <th class="px-4 py-2">Jarak Sumber Air</th>
                            <th class="px-4 py-2">Aksi</th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr v-for="row in jarak_sumberair.data">
                            <td class="border px-4 py-2">{{ row.id }}</td>
                            <td class="border px-4 py-2">{{ row.desa.kel_nama }}</td>
                            <td class="border px-4 py-2">{{ row.kecamatan.kec_nama }}</td>
                            <td class="border px-4 py-2">{{ row.jarak_sumberair }}</td>
                    ...
                    <pagination class="mt-6" :links="jarak_sumberair.links"/>
                    </div>
                    <div v-else>
                                        ...
                                        <div class="">
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <Multiselect v-model="form.id_kelurahan"
                                                             :searchable="true"
                                                             :options="dataDesa"
                                                             placeholder="Pilih Desa/Kelurahan" :disabled="form.id_kecamatan==null"/>
                                            </div>
                                            <div class="mb-4">
                                                <label for="exampleFormControlInput1"
                                                       class="block text-gray-700 text-sm font-bold mb-2">Jarak Sumber Air (Km):                                                    Penduduk:</label>
                                                <input type="number"
                                                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                                       id="exampleFormControlInput1"
                                                       placeholder="Masukkan Jarak Sumber Air"
                                                       v-model="form.jarak_sumberair">
...
export default {
    ...
    props: ['jarak_sumberair', 'kecamatan', 'errors'],
    data() {
        return {
            editMode: false,
            isOpen: false,
    ...
    methods: {
        searchData: function () {
            this.$inertia.get(`/jarak-sumber-air?cari=${this.query}`)
        },
        clearSearch: function () {
            this.query = '';
            this.$inertia.get(`/jarak-sumber-air`)
        },
        getDataDesa(id) {
        ...
        save: function (data) {
            this.$inertia.post('/jarak-sumber-air', data)
            this.reset();
            this.closeModal();
            this.editMode = false;
        },
        ...
        update: function (data) {
            this.$inertia.post('/jarak-sumber-air/' + data.id, {
                id_kelurahan: data.id_kelurahan,
                jarak_sumberair: data.jarak_sumberair,
        ...
        deleteRow: function (data) {
            ...
            data._method = 'DELETE';
            this.$inertia.post('/jarak-sumber-air/' + data.id, data)
            this.reset();
            this.closeModal();
        }
    }
